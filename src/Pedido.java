...
public class Pedido {
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			valorPedido += itensDePedido[i].valorDeVenda();
		}
	...
	public void mesclarPedido(Pedido outroPedido) {
		ItemDePedido[] outrosItens = outroPedido.itensDePedido;
		if (outroPedido.itensDePedido.length + this.itensDePedido.length <= MAX_PRODUTOS) {
			throw new IllegalStateException("Máximo de produtos cadastrados atingido.");
		} else {
			for (int i = 0; i < outrosItens.length; i++) {
				if (outrosItens[i].equals(itensDePedido[i])) {
					int quantidadeAgregada = this.itensDePedido[i].getQuantidade() + outrosItens[i].getQuantidade();
					this.itensDePedido[i].setQuantidade(quantidadeAgregada);
					double menorPreco = outrosItens[i].getPrecoVenda()
					if (itensDePedido[i].getPrecoVenda() < menorPreco) {
						menorPreco = itensDePedido[i].getPrecoVenda();
						itensDePedido[i].setPrecoVenda(menorPreco);
					}
				} else {
					this.itensDePedido[i] = outrosItens[i];
					outrosItens = null;
				}
			}
		}
	...
	public String imprimirRecibo() {
		...
		for (int i = 0; i < itensDePedido.length; i++) {
			double subTotal = itensDePedido[i].getPrecoVenda() * itensDePedido[i].getQuantidade();
			sb.append("Nome: " + itensDePedido[i] + "\n");
			sb.append("Quantidade: " + itensDePedido[i].getQuantidade() + "\n");
			sb.append("Preço unitário: " + itensDePedido[i].getPrecoVenda() + "\n");
			sb.append("Subtotal: R$ " + subTotal + "\n");
			totalPedido += subTotal;
		}
		sb.append("Total geral: R$ " + totalPedido + "\n");
		return sb.toString();
	}
